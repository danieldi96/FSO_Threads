...
int f_pil[9], c_pil[9];		/* posicio de la pilota, en valor enter */
float pos_f[9], pos_c[9];		/* posicio de la pilota, en valor real */
float vel_f[9], vel_c[9];		/* velocitat de la pilota, en valor real */
int nblocs = 0;
int dirPaleta = 0;
...
int main(int n_args, char *ll_args[])
{
	...
	pthread_create(&list_threads[0].thread, NULL, &mou_paleta, &list_threads[0].id);
	for (int i=1; i<actballs+1, i++){
		pthread_create(&list_threads[i].thread, NULL, &mou_pilota,&list_threads[i].id);
	}
	int tem = 0;
	do {
